...
async function startViteChild(): Promise<ViteChild> {
  ...
  while (Date.now() < deadline) {
    ...
    if (child.exitCode !== null) {
      ...
      throw new Error(
        `vite dev exited with code ${child.exitCode} before becoming ready`,
      );
    }
    await Bun.sleep(150);
  }
...
export async function startServer(
  ...
  const authToken =
    normalizeCredential(opts.authToken) ?? loadOrMintLocalAuthToken();
  ...
  if (devMode) {
    console.log(
      "[executor] EXECUTOR_DEV=1 — spawning vite dev child for live UI",
    );
    ...
    console.log(
      `[executor] (internal) vite dev child at ${viteChild.url} — don't open this`,
    );
    serveIndex = () =>
    ...
    const indexFile = Bun.file(
      opts.embeddedWebUI["index.html"] ?? join(clientDir, "index.html"),
    );
...
if (import.meta.main) {
  ...
  console.log(
    `Executor listening on http://localhost:${server.port}/?_token=${server.authToken}`,
  );
}
